...
// 0. Health Check (Untuk memastikan server jalan)
app.get('/api', (req, res) => {
  res.json({ status: "OK", message: "Server Backend Berjalan!", db: isConnected ? "Connected" : "Disconnected" });
});
...
// Handler 404 (Jika rute tidak ditemukan di dalam Express)
app.use((req, res) => {
  res.status(404).json({ message: `Endpoint tidak ditemukan: ${req.originalUrl}` });
});
